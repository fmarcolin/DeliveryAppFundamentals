defmodule DeliveryApp.Orders.CreateOrUpdateTest do
  use ExUnit.Case

  import DeliveryApp.Factory

  alias DeliveryApp.Orders.CreateOrUpdate
  alias DeliveryApp.Users.Agent, as: UserAgent

  describe "call/1" do
    setup do
      DeliveryApp.start_agents()
    ...
    test "when there is no user with given cpf, returns an error", %{item1: item1, item2: item2} do
      params = %{user_cpf: "00000000001", items: [item1, item2]}
    ...
    test "when there are an invalid item, returns an error", %{
      user_cpf: cpf,
      item1: item1,
      item2: item2
    } do
